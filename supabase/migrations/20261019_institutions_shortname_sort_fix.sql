-- ═══ institutions — short_name and sort_order for the eight new rows (hand-applied) ═
--
...
--
-- One of three files, second. They stay three because that is what happened: 19's
...
-- Replaced 18's lowercase slugs with display short names and gave the eight rows a
-- sort_order, all at the bottom of the list (150–220, below every existing university
-- and above Other at 999):
--   …0016 ODTÜ KKK 150 · …0011 İTÜ-KKTC 160 (hyphen) · …0015 KSTÜ 170 · …0010 ALÜ 180
--   …0014 KBÜ 190 · …000f NULL 200 · …0012 NULL 210 · …0013 NULL 220
-- ALÜ, KBÜ and KSTÜ were GUESSES, not the universities' own abbreviations. 20261020
-- clears them and moves all eight rows into the existing order.
--
-- DOES NOT touch name, city or is_active, nor any row outside the eight.
--
-- ─── WHY SECTIONS 0 AND 1 EXIST: THE RECORDED SQL IS NOT SAFE TO RE-RUN ──────
-- Re-running this file after 20 is NOT a no-op: it puts the three guessed short names
-- back and buries the eight rows at 150–220 again. Running it before 18 is worse in the
-- quiet way — every UPDATE matches 0 rows and succeeds, and the file reports nothing.
-- Section 1 refuses both.
...
-- ─── 1. Guard — 18 must have run, and 20 must not have ─────────────────────
DO $$
DECLARE
  r record;
...
BEGIN
  ...
  END IF;
  IF v_state = '1020' THEN
    RAISE EXCEPTION 'REFUSING: superseded by 20261020. Re-running this file would restore the guessed ALÜ/KBÜ/KSTÜ and move the eight rows back to 150–220. Nothing applied.';
  END IF;
...
UPDATE institutions SET short_name = 'ODTÜ KKK', sort_order = 150
  WHERE id = '00000000-0000-4000-b000-000000000016';
UPDATE institutions SET short_name = 'İTÜ-KKTC', sort_order = 160
  WHERE id = '00000000-0000-4000-b000-000000000011';
UPDATE institutions SET short_name = 'KSTÜ',     sort_order = 170
  WHERE id = '00000000-0000-4000-b000-000000000015';
UPDATE institutions SET short_name = 'ALÜ',      sort_order = 180
  WHERE id = '00000000-0000-4000-b000-000000000010';
UPDATE institutions SET short_name = 'KBÜ',      sort_order = 190
  WHERE id = '00000000-0000-4000-b000-000000000014';

UPDATE institutions SET short_name = NULL, sort_order = 200
  WHERE id = '00000000-0000-4000-b000-00000000000f';
UPDATE institutions SET short_name = NULL, sort_order = 210
  WHERE id = '00000000-0000-4000-b000-000000000012';
UPDATE institutions SET short_name = NULL, sort_order = 220
  WHERE id = '00000000-0000-4000-b000-000000000013';
-- ─── END RECORDED ───────────────────────────────────────────────────────────
...
-- ─── 4. State check B — this file's own end state (true until 20261020 runs) ─
DO $$
DECLARE
  r record;
  v_state text := pg_temp.institutions_yodak_state();
BEGIN
  ...
  LOOP
    RAISE NOTICE '  …%  sort=%  short=%  city=%  active=%  %',
      right(r.id::text, 4), r.sort_order, coalesce(r.short_name, 'NULL'), r.city, r.is_active, r.name;
  END LOOP;
  IF v_state IS DISTINCT FROM '1019' THEN
    RAISE EXCEPTION 'the eight rows are in state "%" after this file — expected "1019". The rows are printed above', v_state;
  END IF;
END $$;
...
VALUES ('20261019_institutions_shortname_sort_fix.sql', '28f2257649885e11d951f801800f2111e5d46e270b87bb08cf7443eaf33fccd7')
