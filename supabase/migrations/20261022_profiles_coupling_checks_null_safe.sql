...
CREATE TEMP TABLE tmp_coupling_probe AS SELECT * FROM public.profiles WITH NO DATA;
...
BEGIN
  FOR c IN SELECT * FROM (VALUES
    ...
    END IF;
    EXECUTE format('ALTER TABLE pg_temp.tmp_coupling_probe ADD CONSTRAINT %I %s', c.conname, v_def);
  END LOOP;
  ...
  FOR c IN SELECT * FROM (VALUES
    ...
    BEGIN
      INSERT INTO pg_temp.tmp_coupling_probe (id, resident_status, student_level, institution_id)
      VALUES (gen_random_uuid(), c.resident, c.level, c.inst);
      v_got := true;
...
DROP TABLE pg_temp.tmp_coupling_probe;
...
VALUES ('20261022_profiles_coupling_checks_null_safe.sql', '31d7341d07d9c7f49cd21482ae15e4f3e077d14a67272a807124fee208bea1dd')
