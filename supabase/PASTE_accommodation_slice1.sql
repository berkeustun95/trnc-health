-- ╔══════════════════════════════════════════════════════════════════════════╗
-- ║  PASTE-READY — select ALL of this file and run it in the Supabase SQL    ║
-- ║  editor with Role = postgres. It is                                      ║
-- ║  supabase/migrations/20260904_accommodation_partner_feed.sql             ║
-- ║  plus the ledger stamp. Do NOT hand-edit — edit the migration instead    ║
-- ║  and regenerate, or the checksum stops matching.                         ║
-- ╚══════════════════════════════════════════════════════════════════════════╝
...
-- ─── LEDGER STAMP — the last statement, INSIDE the transaction ──────────────
-- If the paste above is truncated before this line, COMMIT is never reached and
-- NOTHING applies. That is the point: a partial paste cannot half-apply.
--
-- The checksum is of supabase/migrations/20260904_accommodation_partner_feed.sql
-- AS COMMITTED, i.e. without this stamp block. A file cannot contain its own
-- sha256 — verified: appending the stamp changes the hash — so the stamp lives in
-- this paste-ready copy while the committed migration stays the artefact that
-- supabase/migration_ledger_check.sql hashes. Ledger row and disk therefore agree
-- and the check returns zero rows.
INSERT INTO public.schema_migrations_applied (filename, checksum)
VALUES ('20260904_accommodation_partner_feed.sql',
        '3d67911cde0363e47b6ae56aaab81a442362b3ed05f48a2915c1a0f6b96337a0')
