...
const TYPE_ICONS = {
  taxi:             'car-outline',
  car_rental:       'key-outline',
  airport_transfer: 'airplane-outline',
}

const TYPE_LABEL_KEYS = {
  taxi:             'trTypeTaxi',
  car_rental:       'trTypeCarRental',
  airport_transfer: 'trTypeAirportTransfer',
}
...
export default function TransportProfileScreen({ provider, lang, onBack }) {
  ...
  const heroIcon    = TYPE_ICONS[provider.type] ?? 'car-outline'
        ...
        >
          ...
          <View style={s.body}>
            ...
            <View style={s.section}>
              <Text style={s.sectionLabel}>{t('trTypeLabel', lang)}</Text>
              <View style={s.chipRow}>
                <View style={s.chip}>
                  <Ionicons name={heroIcon} size={13} color={colors.primary} />
                  <Text style={s.chipText}>{t(TYPE_LABEL_KEYS[provider.type] ?? 'trTypeTaxi', lang)}</Text>
                </View>
                {provider.type === 'airport_transfer' && provider.airport && (
                  <View style={s.chip}>
                    <Ionicons name="airplane-outline" size={13} color={colors.primary} />
                    <Text style={s.chipText}>{t(AIRPORT_KEYS[provider.airport], lang)}</Text>
                  </View>
                )}
              </View>
            </View>
            ...
            {!!provider.description && (
              <View style={s.section}>
                <Text style={s.sectionLabel}>{t('trAbout', lang)}</Text>
                <Text style={s.description}>{provider.description}</Text>
              </View>
            )}

            {/* Contact */}
            <View style={s.section}>
              <Text style={s.sectionLabel}>{t('trContact', lang)}</Text>
              {provider.phone && (
                <TouchableOpacity
                  style={s.contactRow}
                ...
                >
                  ...
                  <Text style={[s.contactText, { flex: 1 }]}>{provider.phone}</Text>
                  <Text style={s.contactAction}>{t('trCall', lang)}</Text>
                </TouchableOpacity>
              )}
              {provider.whatsapp && (
                <TouchableOpacity
                ...
                >
                  ...
                  <Text style={[s.contactText, { flex: 1 }]}>{provider.whatsapp}</Text>
                  <Text style={[s.contactAction, { color: '#25D366' }]}>{t('trWhatsApp', lang)}</Text>
                </TouchableOpacity>
              )}
            </View>
          </View>
        ...
        <View style={s.ctaWrap}>
          {canCall && (
            ...
            >
              ...
              <Text style={s.ctaText}>{t('trCall', lang)}</Text>
            </TouchableOpacity>
          )}
          {canWA && (
            <TouchableOpacity
            ...
            >
              ...
              <Text style={s.ctaText}>{t('trWhatsApp', lang)}</Text>
            </TouchableOpacity>
          )}
        </View>
      </View>
