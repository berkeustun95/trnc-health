...
export default function App() {
  ...
  //
  // Mirrors the city-welcome trigger below, with one difference that matters: that one has
  // to tell a REAL background -> active apart from an iOS control-centre pull-down, because
  // a stray 'active' would burn a 30-day cooldown. Here the elapsed-time gate does that work
  // — a pull-down is seconds, never 30 minutes — so the clock is the guard rather than the
  // state machine.
  //
  // A re-check that comes back null CLEARS a force block. That is the fail-open requirement,
  // not an oversight: if the server is unreachable on the second look, the user gets their
  // app back. A client-side force-update is a prompt, never a security boundary.
  useEffect(() => {
    let cancelled = false
  ...
  // was the first design and it dead-ends iOS: tapping the emergency sheet's BACKDROP fires
  // SheetOverlay.onDismiss -> setShowEmergencyModal(false), while the separate escape state
  // stays 'emergency' — so the modal stays hidden, the selector renders the neutral backdrop,
  // and the user is on a blank screen. Android back rescues it; iOS has no hardware back.
  // Derived, every close path (backdrop, X, onBack, hardware back) re-shows the modal for
  // free, because visibility is computed from the flags those paths already set.
  //
  // Note it is `updateTier === 'force'` that gates the SELECTOR block below, not this — when
  // the escape is open we must still be inside that block, rendering the duty screen, rather
  // than falling through to the rest of the app. This gates only the MODAL.
  ...
  // welcome and auth branches. That last part is not a preference: a signed-out user tapping
  // the emergency escape would otherwise land on WelcomeScreen behind a hidden modal.
  //
  // ONE READABLE ALLOW-LIST, exactly like the gateActive block below, for the reason its
  ...
  // a module added later could widen it by accident. Nothing not named here is reachable —
  // tabs, deep links and every module are simply not rendered. A duty push deep link while
  // blocked lands on the roster, which is the exempt screen anyway.
  //
  ...
  return (
    <SafeAreaProvider>
      ...
      {/* In the FINAL return, next to PolicyUpdateNotice — never in the content selector.
          ...
          above it), but this Modal lives out here and knows nothing about onboarding, so
